...
class ComfyUI:
    ...
    def queue_prompt(self, prompt):
        ...
        if http_error:
            raise Exception(
                f"ComfyUI Error - Your workflow could not be run. This usually happens if you’re trying to use an unsupported node. Error: {err_msg}"  # noqa
            )
            ...
            if isinstance(out, str):
                ...
                if message["type"] == "executing":
                    ...
                    if data["node"] is None and data["prompt_id"] == prompt_id:
                        ...
                        print(
                            f"Executing node {data['node']}, title: {meta.get('title', 'Unknown')}, class type: {class_type}"  # noqa
                        )
            else:
                continue
    ...
    def load_workflow(self, workflow):
        ...
        if any(key in wf.keys() for key in ["last_node_id", "last_link_id", "version"]):
            raise ValueError(
                "You need to use the API JSON version of a ComfyUI workflow. To do this go to your ComfyUI settings and turn on 'Enable Dev mode Options'. Then you can save your ComfyUI workflow via the 'Save (API Format)' button."  # noqa
            )
    ...
    def reset_execution_cache(self):
        ...
        with open("constants/reset.json", "r") as file:
